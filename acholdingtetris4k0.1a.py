...
OST accuracy:
  - A-Type (Korobeiniki): 160 BPM exact.
    BT = 0.1875 s per eighth-note.  (Old BT=0.15 was 200 BPM — too fast.)
  ...
  - Full A-A-B-A Korobeiniki loop (~48s) before pygame seamlessly repeats.
  - Music plays ONLY during active gameplay, never menus/level-select.
"""
...
# Full frequency table — octaves 3-5 + enharmonics
NF = {
...
# Transcribed from the original DMG-TCRE-0 ROM.

_PA = [   # Part A — classic verse (both phrases identical, matching ROM)
...
_PB = [   # Part B — bridge (higher register)
    ('D5',2),('F5',1),('A5',1),('G5',2),('F5',1),('E5',1),
    ('C5',2),('E5',1),('D5',1),('C5',2),('B4',2),
    ('B4',1),('C5',1),('D5',3),('E5',2),
    ('C5',2),('A4',2),('A4',4),
...
MEL_A = _PA + _PA + _PB + _PA   # A→A→B→A before seamless loop
...
def _build_ost(m_data):
    ...
    lead = array.array('h')
    bass = array.array('h')
    sil0 = array.array('h')   # placeholder; built per-note
...
class App:
    def __init__(self):
        ...
        self.osts={}; self.sfx={}
        if self.aok:
            print("Synthesizing — 160 BPM, 25% duty, GB articulation…")
