...
class App:
    def __init__(self):
        ...
        pygame.display.set_caption("Game Boy TETRIS")
        ...
        # Music does NOT start here (only during gameplay)
        # self._mstart()  # Removed
    ...
    def _evt(self, e):
        ...
        if self.state == 'menu':
            if k == pygame.K_UP: self.mi = max(0, self.mi - 1); self._psfx('menu')
                ...
                self.mtype = opts[idx]
                self._psfx('menu')
                # Do NOT start music here; only when game starts
                # self._mstop() and self._mstart() removed
            ...
            if k == pygame.K_LEFT: self.lsel = max(0, self.lsel - 1); self._psfx('menu')
                ...
                self.state = 'game'
                g.start(self.lsel)
                # Start music only when game begins (if not OFF)
                if self.mtype != 'OFF':
                    self._mstart()
            ...
            if g.gameover and not g.go_anim:
                if k == pygame.K_RETURN:
                    self.state = 'menu'
                    # Music already stopped by gameover, but ensure it's off
                    self._mstop()
                return
